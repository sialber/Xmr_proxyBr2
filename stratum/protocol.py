...
class Protocol(LineOnlyReceiver):
    ...
    def writeJsonRequest(self, method, params, is_notification=False):
        ...
        serialized = json.dumps({'id': request_id, 'method': method, 'params': params, 'jsonrpc':'2.0'})
    ...
    def writeJsonResponse(self, data, message_id, use_signature=False, sign_method='', sign_params=[]):
        ...
        if use_signature:
            ...
            serialized = json.dumps({'id': message_id, 'resultado': data, 'erro': None, 'jsonrpc':'2.0'})
    ...
    def writeJsonError(self, code, message, traceback, message_id, use_signature=False, sign_method='', sign_params=[]):       
        if use_signature:
            ...
            serialized = json.dumps({'id': message_id, 'resultado': None, 'erro': (code, message, traceback)})
    ...
    def lineReceived(self, line, request_counter):
        ...
        try:
            ...
            #self.writeGeneralError("Não é possível decodificar a mensagem '%s'" % line)
            request_counter.finish()
            raise custom_exceptions.ProtocolException("Não é possível decodificar a mensagem '%s'" % line.strip())
