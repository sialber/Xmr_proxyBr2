...
if __name__ == '__main__':
    if (settings.PAYMENT_ID and len(settings.PAYMENT_ID)!=64) or (len(settings.WALLET)!=95 and len(settings.WALLET)!=106):
        log.error("Carteira ou PAYMENT_ID incorretos !!!")
        quit()
...
def on_connect(f):
    ...
    log.info("Conectado a Stratum pool as %s:%d" % f.main_host)
...
def on_disconnect(f):
    ...
    log.info("Disconectado da Stratum pool as %s:%d" % f.main_host)
...
def main():
    ...
    log.warning("Tentando se conectar a Stratum pool as %s:%d" % (pool_host, pool_port))
    ...
    try:
        ...
        log.warning("O Pool Primaria deve estar conectada pela primeira vez para iniciar o failover")
        return
    ...
    if settings.STRATUM_HOST == '0.0.0.0':
        log.warning("PROXY esta escutando todos os IPS na porta %d (stratum)" % settings.STRATUM_PORT)
    else:
